...
The Spielverlagerung essay is an argument, not a dataset. Before we can test it we must state precisely what it asserts. We read the article and distil its reasoning into four hypotheses; each is introduced by the mechanism the article proposes and closed by the prediction we will test in Section 5.

### 4.1 H1 — Perception: the diagonal arrives where the defender is blind
...
The same aggregation, computed per opponent rather than over the whole sample, becomes a scouting instrument: a picture of where a specific team's defenders habitually turn away from danger, and therefore where to attack them diagonally. And computed per player, it becomes a profile. Figure 9 shows one — a single player's passing read through the diagonality lens, the kind of per-player, per-opponent output the framework produces directly.
...
**Figure 9. Player application: Michael Olise, Bayern 5–0 Hamburg.** Pass map coloured by DFL direction class — a per-player, per-opponent diagonal profile produced directly by the framework.
