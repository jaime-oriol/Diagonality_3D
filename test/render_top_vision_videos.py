...
Same pipeline as `render_kane_goal.py` (vision_plot.plot_vision_frame
with full FOV + occlusion at smoothing=7) but parameterized to render
ANY event from `outputs/tables/top_dos_events.json`. The focus player
...
def main():
    ...
    print(f"Loaded {EVENT_LIST}: rendering top {len(events)} as VISION videos")
